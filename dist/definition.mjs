...
export const lstCollectionFields = [
    {
        ...
        fields: [
            ...
            { name: 'GSTRegistrationNumber', datatype: 'string', description: 'company GSTIN, if set on the company master (may be blank if the client stores it only in the GST registration details)' },
    ...
    },
    {
        ...
        fields: [
            ...
            { name: 'PartyGSTIN', datatype: 'string', description: 'GST registration number (GSTIN) of the party ledger, if set on the master' },
            { name: 'RegisteredGSTIN', datatype: 'string', expression: 'if NOT $$IsEmpty:$PartyGSTIN then $PartyGSTIN else $$CollectionField:$GSTIN:1:LedGSTRegDetails', description: 'active party GSTIN: the PartyGSTIN scalar when present, otherwise the GSTIN from the dated GST-registration sub-list (LedGSTRegDetails). Use this for GSTR-1 B2B party-to-GSTIN mapping' },
            { name: 'GSTRegistrationType', datatype: 'string', description: 'GST registration type of the party: Regular, Composition, Unregistered/Consumer, etc. Maps to Zoho gst_treatment' },
            { name: 'LedgerStateName', datatype: 'string', description: 'state of the party (place of supply). Maps to Zoho contact billing_address state / place_of_contact' },
            { name: 'Pincode', datatype: 'string', description: 'postal / PIN code of the party mailing address' },
            { name: 'LedgerAddress', datatype: 'string', expression: 'if $$IsEmpty:$Address then "" else $$FullList:Address:$Address', description: 'full mailing address of the party (address lines joined). Maps to Zoho contact billing_address' },
            { name: 'Email', datatype: 'string', description: 'email address of the party, if set on the master' },
            { name: 'LedgerPhone', datatype: 'string', description: 'phone / mobile number of the party, if set on the master' },
            { name: 'LedgerContact', datatype: 'string', description: 'contact person name of the party, if set on the master' },
            { name: 'BillCreditPeriod', datatype: 'number', expression: 'if $$IsEmpty:$BillCreditPeriod then 0 else $$Number:$BillCreditPeriod', description: 'credit period allowed to the party in days (bill-wise). Maps to Zoho payment_terms' }
        ]
    },
    {
        collection: 'Unit',
    ...
    },
    {
        ...
        fields: [
            ...
            { name: 'HSNCode', datatype: 'string', expression: '$InfGSTHSNCode', description: 'HSN (goods) or SAC (services) code inferred for the item (own GST details or inherited from stock group / company). Maps to Zoho item hsn_or_sac' },
            { name: 'HSNDescription', datatype: 'string', expression: '$InfGSTHSNDescription', description: 'HSN/SAC description inferred for the item' },
            { name: 'GSTRate', datatype: 'number', expression: 'if $$IsEmpty:$InfGSTIGSTRate then 0 else $$Number:$InfGSTIGSTRate', description: 'inferred IGST rate percent (= total GST rate; CGST+SGST each = half). Maps to Zoho item tax rate' },
            { name: 'Taxability', datatype: 'string', expression: '$InfGSTTaxablility', description: 'GST taxability inferred for the item: Taxable, Exempt, Nil Rated, Non-GST' },
            { name: 'TypeOfSupply', datatype: 'string', expression: 'if $$IsEmpty:$GSTMSTTypeofSupply then "" else $GSTMSTTypeofSupply', description: 'type of supply: Goods or Services. Maps to Zoho item product_type' },
        ]
    },
    {
        collection: 'Bill',
        ...
        fields: [
            ...
            { name: 'DueDate', datatype: 'date', expression: 'if $$IsEmpty:$BillCreditPeriod then "" else $$PyrlYYYYMMDDFormat:$BillCreditPeriod:"-"', description: 'effective due date of the bill (bill date + credit period). Maps to Zoho opening invoice/bill due_date' },
            { name: 'BillPartyGSTIN', datatype: 'string', expression: '$PartyGSTIN:Ledger:$Parent', description: 'GSTIN of the party the bill belongs to (from the party ledger master). Use for Zoho opening AR/AP contact mapping' }
        ]
    }
];
...
export const lstReportXml = new Map([
    ...
    ['day-book', '<ENVELOPE><HEADER><VERSION>1</VERSION><TALLYREQUEST>Export</TALLYREQUEST><TYPE>Data</TYPE><ID>MyTallyLiveReport</ID></HEADER><BODY><DESC><STATICVARIABLES><SVEXPORTFORMAT>$$SysName:XML</SVEXPORTFORMAT><SVFROMDATE>{{ fromDate | formatDate("d-MMM-yyyy") }}</SVFROMDATE><SVTODATE>{{ toDate | formatDate("d-MMM-yyyy") }}</SVTODATE>{% if targetCompany %}<SVCURRENTCOMPANY>{{ targetCompany | escape }}</SVCURRENTCOMPANY>{% endif %}</STATICVARIABLES><TDL><TDLMESSAGE><REPORT NAME="MyTallyLiveReport"><FORMS>MyForm</FORMS></REPORT><FORM NAME="MyForm"><PARTS>MyPart01</PARTS><XMLTAG>DATA</XMLTAG></FORM><PART NAME="MyPart01"><LINES>MyLine01</LINES><REPEAT>MyLine01 : MyCollection</REPEAT><SCROLLED>Vertical</SCROLLED></PART><LINE NAME="MyLine01"><FIELDS>FldDate,FldVchType,FldVchNo,FldParty,FldAmount,FldNarration</FIELDS><XMLTAG>ROW</XMLTAG></LINE><FIELD NAME="FldDate"><SET>$Date</SET><XMLTAG>date</XMLTAG></FIELD><FIELD NAME="FldVchType"><SET>$VoucherTypeName</SET><XMLTAG>voucher_type</XMLTAG></FIELD><FIELD NAME="FldVchNo"><SET>$VoucherNumber</SET><XMLTAG>voucher_number</XMLTAG></FIELD><FIELD NAME="FldParty"><SET>if $$IsEmpty:$PartyLedgerName then $PartyName else $PartyLedgerName</SET><XMLTAG>party_ledger</XMLTAG></FIELD><FIELD NAME="FldAmount"><SET>if $$IsDebit:$Amount then -$$NumValue:$Amount else $$NumValue:$Amount</SET><XMLTAG>amount</XMLTAG></FIELD><FIELD NAME="FldNarration"><SET>$Narration</SET><XMLTAG>narration</XMLTAG></FIELD><COLLECTION NAME="MyCollection"><TYPE>Voucher</TYPE><FETCH>Date,VoucherTypeName,VoucherNumber,PartyLedgerName,PartyName,Narration,Amount</FETCH><FILTER>FilterExcludeOrderVch,FilterCancelledVouchers,FilterOptionalVouchers{% if voucherType %},FilterVoucherType{% endif %}</FILTER></COLLECTION><SYSTEM TYPE="Formulae" NAME="FilterExcludeOrderVch">NOT $$IsOrderVch:$VoucherTypeName</SYSTEM><SYSTEM TYPE="Formulae" NAME="FilterCancelledVouchers">NOT $IsCancelled</SYSTEM><SYSTEM TYPE="Formulae" NAME="FilterOptionalVouchers">NOT $IsOptional</SYSTEM>{% if voucherType %}<SYSTEM TYPE="Formulae" NAME="FilterVoucherType">$$IsEqual:$VoucherTypeName:\"{{ voucherType | escape }}\"</SYSTEM>{% endif %}</TDLMESSAGE></TDL></DESC></BODY></ENVELOPE>'],
    ['voucher-register', '<ENVELOPE><HEADER><VERSION>1</VERSION><TALLYREQUEST>Export</TALLYREQUEST><TYPE>Data</TYPE><ID>MyTallyLiveReport</ID></HEADER><BODY><DESC><STATICVARIABLES><SVEXPORTFORMAT>$$SysName:XML</SVEXPORTFORMAT><SVFROMDATE>{{ fromDate | formatDate("d-MMM-yyyy") }}</SVFROMDATE><SVTODATE>{{ toDate | formatDate("d-MMM-yyyy") }}</SVTODATE>{% if targetCompany %}<SVCURRENTCOMPANY>{{ targetCompany | escape }}</SVCURRENTCOMPANY>{% endif %}</STATICVARIABLES><TDL><TDLMESSAGE><REPORT NAME="MyTallyLiveReport"><FORMS>MyForm</FORMS></REPORT><FORM NAME="MyForm"><PARTS>MyPart01</PARTS><XMLTAG>DATA</XMLTAG></FORM><PART NAME="MyPart01"><LINES>MyLine01</LINES><REPEAT>MyLine01 : MyCollection</REPEAT><SCROLLED>Vertical</SCROLLED></PART><LINE NAME="MyLine01"><FIELDS>FldParent</FIELDS><EXPLODE>MyPart02</EXPLODE></LINE><FIELD NAME="FldParent"><SET>\"\"</SET></FIELD><PART NAME="MyPart02"><LINES>MyLine02</LINES><REPEAT>MyLine02 : AllLedgerEntries</REPEAT><SCROLLED>Vertical</SCROLLED></PART><LINE NAME="MyLine02"><FIELDS>FldDate,FldVchType,FldVchNo,FldParty,FldPOS,FldLedger,FldGroup,FldAmount,FldDrCr,FldNarration</FIELDS><XMLTAG>ROW</XMLTAG></LINE><FIELD NAME="FldDate"><SET>$Date</SET><XMLTAG>date</XMLTAG></FIELD><FIELD NAME="FldVchType"><SET>$VoucherTypeName</SET><XMLTAG>voucher_type</XMLTAG></FIELD><FIELD NAME="FldVchNo"><SET>$VoucherNumber</SET><XMLTAG>voucher_number</XMLTAG></FIELD><FIELD NAME="FldParty"><SET>if $$IsEmpty:$PartyLedgerName then $PartyName else $PartyLedgerName</SET><XMLTAG>party_ledger</XMLTAG></FIELD><FIELD NAME="FldPOS"><SET>$PlaceOfSupply</SET><XMLTAG>place_of_supply</XMLTAG></FIELD><FIELD NAME="FldLedger"><SET>$LedgerName</SET><XMLTAG>ledger_name</XMLTAG></FIELD><FIELD NAME="FldGroup"><SET>$Parent:Ledger:$LedgerName</SET><XMLTAG>ledger_group</XMLTAG></FIELD><FIELD NAME="FldAmount"><SET>if $$IsDebit:$Amount then -$$NumValue:$Amount else $$NumValue:$Amount</SET><XMLTAG>amount</XMLTAG></FIELD><FIELD NAME="FldDrCr"><SET>if $$IsDebit:$Amount then \"DR\" else \"CR\"</SET><XMLTAG>dr_cr</XMLTAG></FIELD><FIELD NAME="FldNarration"><SET>$Narration</SET><XMLTAG>narration</XMLTAG></FIELD><COLLECTION NAME="MyCollection"><TYPE>Voucher</TYPE><FETCH>Date,VoucherTypeName,VoucherNumber,PartyLedgerName,PartyName,PlaceOfSupply,LedgerName,Amount,Narration,AllLedgerEntries</FETCH><FILTER>FilterExcludeOrderVch,FilterCancelledVouchers,FilterOptionalVouchers{% if voucherType %},FilterVoucherType{% endif %}{% if voucherNumber %},FilterVoucherNumber{% endif %}</FILTER></COLLECTION><SYSTEM TYPE="Formulae" NAME="FilterExcludeOrderVch">NOT $$IsOrderVch:$VoucherTypeName</SYSTEM><SYSTEM TYPE="Formulae" NAME="FilterCancelledVouchers">NOT $IsCancelled</SYSTEM><SYSTEM TYPE="Formulae" NAME="FilterOptionalVouchers">NOT $IsOptional</SYSTEM>{% if voucherType %}<SYSTEM TYPE="Formulae" NAME="FilterVoucherType">$$IsEqual:$VoucherTypeName:\"{{ voucherType | escape }}\"</SYSTEM>{% endif %}{% if voucherNumber %}<SYSTEM TYPE="Formulae" NAME="FilterVoucherNumber">$$IsEqual:$VoucherNumber:\"{{ voucherNumber | escape }}\"</SYSTEM>{% endif %}</TDLMESSAGE></TDL></DESC></BODY></ENVELOPE>'],
]);
...
export const lstPushXml = new Map([
    ...
    ['voucher-create', '<ENVELOPE><HEADER><VERSION>1</VERSION><TALLYREQUEST>Import</TALLYREQUEST><TYPE>Data</TYPE><ID>Vouchers</ID></HEADER><BODY><DESC><STATICVARIABLES><SVEXPORTFORMAT>$$SysName:XML</SVEXPORTFORMAT>{% if targetCompany %}<SVCURRENTCOMPANY>{{ targetCompany | escape }}</SVCURRENTCOMPANY>{% endif %}</STATICVARIABLES></DESC><DATA><TALLYMESSAGE xmlns:UDF="TallyUDF"><VOUCHER REMOTEID="{{ remoteId | escape }}" VCHTYPE="{{ voucherType | escape }}" ACTION="Create" OBJVIEW="Accounting Voucher View"><DATE>{{ date | formatDate("yyyyMMdd") }}</DATE><EFFECTIVEDATE>{{ date | formatDate("yyyyMMdd") }}</EFFECTIVEDATE><VOUCHERTYPENAME>{{ voucherType | escape }}</VOUCHERTYPENAME>{% if voucherNumber %}<VOUCHERNUMBER>{{ voucherNumber | escape }}</VOUCHERNUMBER>{% endif %}{% if partyLedger %}<PARTYLEDGERNAME>{{ partyLedger | escape }}</PARTYLEDGERNAME>{% endif %}{% if reference %}<REFERENCE>{{ reference | escape }}</REFERENCE>{% endif %}{% if narration %}<NARRATION>{{ narration | escape }}</NARRATION>{% endif %}{% for e in entries %}<ALLLEDGERENTRIES.LIST><LEDGERNAME>{{ e.ledger | escape }}</LEDGERNAME><ISDEEMEDPOSITIVE>{{ \"Yes\" if e.isDebit else \"No\" }}</ISDEEMEDPOSITIVE><AMOUNT>{{ e.amount }}</AMOUNT></ALLLEDGERENTRIES.LIST>{% endfor %}</VOUCHER></TALLYMESSAGE></DATA></BODY></ENVELOPE>'],
    ['voucher-delete', '<ENVELOPE><HEADER><VERSION>1</VERSION><TALLYREQUEST>Import</TALLYREQUEST><TYPE>Data</TYPE><ID>Vouchers</ID></HEADER><BODY><DESC><STATICVARIABLES><SVEXPORTFORMAT>$$SysName:XML</SVEXPORTFORMAT>{% if targetCompany %}<SVCURRENTCOMPANY>{{ targetCompany | escape }}</SVCURRENTCOMPANY>{% endif %}</STATICVARIABLES></DESC><DATA><TALLYMESSAGE xmlns:UDF="TallyUDF"><VOUCHER REMOTEID="{{ remoteId | escape }}" ACTION="Delete" VCHTYPE="{{ voucherType | escape }}"><DATE>{{ date | formatDate(\"yyyyMMdd\") }}</DATE><VOUCHERTYPENAME>{{ voucherType | escape }}</VOUCHERTYPENAME></VOUCHER></TALLYMESSAGE></DATA></BODY></ENVELOPE>'],
    ['voucher-invoice', '<ENVELOPE><HEADER><VERSION>1</VERSION><TALLYREQUEST>Import</TALLYREQUEST><TYPE>Data</TYPE><ID>Vouchers</ID></HEADER><BODY><DESC><STATICVARIABLES><SVEXPORTFORMAT>$$SysName:XML</SVEXPORTFORMAT>{% if targetCompany %}<SVCURRENTCOMPANY>{{ targetCompany | escape }}</SVCURRENTCOMPANY>{% endif %}</STATICVARIABLES></DESC><DATA><TALLYMESSAGE xmlns:UDF="TallyUDF"><VOUCHER REMOTEID="{{ remoteId | escape }}" VCHTYPE="{{ voucherType | escape }}" ACTION="Create"><DATE>{{ date | formatDate(\"yyyyMMdd\") }}</DATE><EFFECTIVEDATE>{{ date | formatDate(\"yyyyMMdd\") }}</EFFECTIVEDATE><VOUCHERTYPENAME>{{ voucherType | escape }}</VOUCHERTYPENAME><PARTYLEDGERNAME>{{ partyLedger | escape }}</PARTYLEDGERNAME>{% if voucherNumber %}<VOUCHERNUMBER>{{ voucherNumber | escape }}</VOUCHERNUMBER>{% endif %}{% if reference %}<REFERENCE>{{ reference | escape }}</REFERENCE>{% endif %}{% if narration %}<NARRATION>{{ narration | escape }}</NARRATION>{% endif %}<ISINVOICE>Yes</ISINVOICE>{% for it in items %}<ALLINVENTORYENTRIES.LIST><STOCKITEMNAME>{{ it.stockItemName | escape }}</STOCKITEMNAME><ISDEEMEDPOSITIVE>{{ it.deemedPositive }}</ISDEEMEDPOSITIVE><RATE>{{ it.rate }}/{{ it.unit | escape }}</RATE><AMOUNT>{{ it.amountStr }}</AMOUNT><ACTUALQTY>{{ it.qty }} {{ it.unit | escape }}</ACTUALQTY><BILLEDQTY>{{ it.qty }} {{ it.unit | escape }}</BILLEDQTY><ACCOUNTINGALLOCATIONS.LIST><LEDGERNAME>{{ it.itemLedger | escape }}</LEDGERNAME><ISDEEMEDPOSITIVE>{{ it.deemedPositive }}</ISDEEMEDPOSITIVE><AMOUNT>{{ it.amountStr }}</AMOUNT></ACCOUNTINGALLOCATIONS.LIST></ALLINVENTORYENTRIES.LIST>{% endfor %}<LEDGERENTRIES.LIST><LEDGERNAME>{{ partyLedger | escape }}</LEDGERNAME><ISDEEMEDPOSITIVE>{{ partyDeemedPositive }}</ISDEEMEDPOSITIVE><AMOUNT>{{ partyAmountStr }}</AMOUNT></LEDGERENTRIES.LIST>{% for t in taxes %}<LEDGERENTRIES.LIST><LEDGERNAME>{{ t.ledger | escape }}</LEDGERNAME><ISDEEMEDPOSITIVE>{{ t.deemedPositive }}</ISDEEMEDPOSITIVE><AMOUNT>{{ t.amountStr }}</AMOUNT></LEDGERENTRIES.LIST>{% endfor %}</VOUCHER></TALLYMESSAGE></DATA></BODY></ENVELOPE>']
]);
