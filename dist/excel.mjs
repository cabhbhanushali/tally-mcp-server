...
export function parseVoucherWorkbook(filePath, sheetName) {
    const result = { vouchers: [], errors: [] };
    ...
    rows.forEach((raw, i) => {
        ...
        for (const [rawKey, canon] of headerMap)
            rec[canon] = raw[rawKey];
        rec.__row = i + 2; // 1-based + header
        const id = String(rec.voucherId ?? '').trim();
        if (!id) {
            result.errors.push(`Row ${rec.__row}: blank voucher_id`);
            return;
        }
        if (!groups.has(id))
            groups.set(id, []);
    ...
    for (const [id, grp] of groups) {
        ...
        if (!date) {
            result.errors.push(`Voucher ${id}: invalid or missing date "${first.date}"`);
            continue;
        }
        const voucherType = String(first.voucherType ?? '').trim();
        if (!voucherType) {
            result.errors.push(`Voucher ${id}: missing voucher_type`);
            continue;
        }
        const entries = [];
        let dr = 0, cr = 0;
        ...
        for (const r of grp) {
            ...
            if (!ledger) {
                result.errors.push(`Row ${r.__row} (voucher ${id}): blank ledger`);
                bad = true;
                break;
            }
            if (debit > 0 && credit > 0) {
                result.errors.push(`Row ${r.__row} (voucher ${id}): both debit and credit set`);
                bad = true;
                break;
            }
            if (debit === 0 && credit === 0) {
                result.errors.push(`Row ${r.__row} (voucher ${id}): both debit and credit are zero`);
                bad = true;
                break;
            }
        ...
        if (bad)
            continue;
        if (entries.length < 2) {
            result.errors.push(`Voucher ${id}: needs at least 2 ledger lines`);
            continue;
        }
