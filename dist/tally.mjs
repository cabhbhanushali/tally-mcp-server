 ...
 */
export async function fetchNestedWalk(routePath, fields, filters, fetchList, statics) {
    ...
    if (statics.fromDate)
        staticsXml += `<SVFROMDATE>${utility.Date.format(statics.fromDate, 'd-MMM-yyyy')}</SVFROMDATE>`;
    if (statics.toDate)
        staticsXml += `<SVTODATE>${utility.Date.format(statics.toDate, 'd-MMM-yyyy')}</SVTODATE>`;
